...
int get(const int i, const uint edgemask, const int endpoints, const uint info) {
  MII& M = dp[i+1][edgemask][endpoints];
  auto it = M.find(info);
  ...
  for (int j=0;j<=d;++j) { // right edge
    ...
    for (int jj=0;jj<(j?j:1);++jj) { // 2nd right edge
      ...
      for (int k=0;k<=2;++k) { // left edges
        ...
        x=max(x,degree+get(ni,nedgemask,nendpoints,ninfo));
      }
    }
  }
  return x;
